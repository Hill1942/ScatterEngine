...
int SELoader::Reset()
{
	...
	strcpy(comment, SELOADER_DEFAULT_COMMENT);

	return 1;
}

int SELoader::Open(char* filename)
{
	Reset();
...
int SELoader::SetComment(char* string)
{
	if (strlen(string) < SELOADER_MAX_COMMENT)
	{
...
char* SELoader::Getline(int mode)
{
	char* string;

	if (file)
	{
		if (mode & SELOADER_STRIP_EMPTY_LINES)
		{
			while (1)
			{
				if ((string = fgets(buffer, SELOADER_BUFFER_SIZE, file)) == NULL)
					break;
				...
				if ((slength - sindex ) > 0)
				{
					...
					if (mode & SELOADER_STRIP_COMMENTS)
					{
		...
		{
			string = fgets(buffer, SELOADER_BUFFER_SIZE, file);
		}

		if (string)
		{
			numLines++;
			if (mode & SELOADER_STRIP_WS_ENDS)
			{
...
int SELoader::PatternMatch(char* string, char* pattern, ...)
{
						   ...
						   int          flags)
{
	...
	if (!SELOADER.Open(filename))
	{
		//MessageBox(NULL, "Couldn't open .asc file", "Error", MB_OK);
		std::cout << "Couldn't open .asc file" << std::endl;
		return 0;
	}
	while (1)
	{
		if (!SELOADER.Getline(SELOADER_STRIP_EMPTY_LINES | SELOADER_STRIP_WS_ENDS))
		{
			//MessageBox(NULL, "No name in .asc file", "Error", MB_OK);
			std::cout << "No name in .asc file" << std::endl;
			return 0;
		}
		if (SELOADER.PatternMatch(SELOADER.buffer, "['Named'] ['object:']"))
		{
			strcpy(tokenBuffer, SELOADER.buffer);
			strcpy(seperators, "\"");
	...
	while (1)
	{
		if (!SELOADER.Getline(SELOADER_STRIP_COMMENTS | SELOADER_STRIP_WS_ENDS))
		{
			//MessageBox(NULL, "No Tri-mesh line in .asc file", "Error", MB_OK);
			std::cout << "No Tri-mesh line in .asc file" << std::endl;
			return 0;
		}
		if (SELOADER.PatternMatch(SELOADER.buffer, "['Tri-mesh,'] ['Vertices:'] [i] ['Faces:'] [i]"))
		{
			obj->numVertics = SELOADER.pInts[0];
			obj->numPolys   = SELOADER.pInts[1];
			//MessageBox(NULL, "No Tri-mesh line in .asc file", "Error", MB_OK);
			break;
		}
	}
	...
	while (1)
	{
		if (!SELOADER.Getline(SELOADER_STRIP_COMMENTS | SELOADER_STRIP_WS_ENDS))
		{
			...
			std::cout << "No 'Vertex list' line in .asc file" << std::endl;
			return 0;
		}
		if (SELOADER.PatternMatch(SELOADER.buffer, "['Vertex'] ['list:']"))
		{
			break;
		}
	}
	...
	for (int vertex = 0; vertex < obj->numVertics; vertex++)
	{
		while (1)
		{
			if (!SELOADER.Getline(SELOADER_STRIP_COMMENTS | SELOADER_STRIP_WS_ENDS))
			{
				...
				std::cout << "Vertex list end abruptly" << std::endl;
				return 0;
			}
			StripChars(SELOADER.buffer, SELOADER.buffer, ":XYZ");
			if (SELOADER.PatternMatch(SELOADER.buffer, "['Vertex'] [i] [f] [f] [f]"))
			{
				obj->vLocalList[vertex].x = SELOADER.pFloats[0];
				obj->vLocalList[vertex].y = SELOADER.pFloats[1];
				obj->vLocalList[vertex].z = SELOADER.pFloats[2];
	...
	while (1)
	{
		if (!SELOADER.Getline(SELOADER_STRIP_EMPTY_LINES | SELOADER_STRIP_WS_ENDS))
		{
			...
			std::cout << "Face list not found" << std::endl;
			return 0;
		}
		if (SELOADER.PatternMatch(SELOADER.buffer, "['Face'] ['list:']"))
		{
	...
	for (int poly = 0; poly < obj->numPolys; poly++)
	{
		while (1)
		{
			if (!SELOADER.Getline(SELOADER_STRIP_EMPTY_LINES | SELOADER_STRIP_WS_ENDS))
			{
				...
				std::cout << "Face list end abruptly" << std::endl;
				return 0;
			}
			StripChars(SELOADER.buffer, SELOADER.buffer, ":ABC");

			if (SELOADER.PatternMatch(SELOADER.buffer, "['Face'] [i] [i] [i] [i]"))
			{
				if (flags & VERTEX_FLAGS_INVERT_WINDING_ORDER)
				{
					obj->polyList[poly].vert[0] = SELOADER.pInts[3];
					obj->polyList[poly].vert[1] = SELOADER.pInts[2];
					obj->polyList[poly].vert[2] = SELOADER.pInts[1];
				}
				else
				{
					obj->polyList[poly].vert[0] = SELOADER.pInts[1];
					obj->polyList[poly].vert[1] = SELOADER.pInts[2];
					obj->polyList[poly].vert[2] = SELOADER.pInts[3];
				}
		...
		while (1)
		{
			if (!SELOADER.Getline(SELOADER_STRIP_EMPTY_LINES | SELOADER_STRIP_WS_ENDS))
			{
				...
				std::cout << "Material list end abruptly" << std::endl;
				return 0;
			}
			ReplaceChars(SELOADER.buffer, SELOADER.buffer, ":\"rgba", ' ');
			if (SELOADER.PatternMatch(SELOADER.buffer, "[i] [i] [i]"))
			{
				r = SELOADER.pInts[0];
				g = SELOADER.pInts[1];
				b = SELOADER.pInts[2];

				obj->polyList[poly].color = RGB32BIT_8888(255, r, g, b);
